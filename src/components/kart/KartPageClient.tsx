...
import { formatBydelLabel, normalizeHashtag, hashtagsMatch, formatCheckTimeLabel, toLocalDatetimeInput } from "@/lib/utils";
...
export function KartPageClient({ initialPins }: { initialPins: MapPinSummary[] }) {
  ...
  const availableHashtags = useMemo(() => {
    ...
    for (const tag of POPULAR_HASHTAGS) tags.add(normalizeHashtag(tag));
    for (const pin of pins) {
      for (const tag of pin.hashtags) tags.add(normalizeHashtag(tag));
    }
    return [...tags].filter(Boolean).sort((a, b) => a.localeCompare(b, "nb"));
  }, [pins]);

  const solTimeLabel = formatCheckTimeLabel(checkTime);

  const baseFilteredPins = useMemo(() => {
    return pins.filter((pin) => {
      if (bydelFilter && pin.bydel !== bydelFilter) return false;
      if (hashtagFilter && !pin.hashtags.some((tag) => hashtagsMatch(tag, hashtagFilter))) return false;
  ...
  const emptyListMessage = solNowOnly
    ? `Ingen steder med sol ${solTimeLabel === "nå" ? "akkurat nå" : `kl. ${solTimeLabel}`}.`
    : hasActiveFilters
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <p className="mt-2 text-xs text-oslo-muted">
            ...
            {solNowOnly ? ` med sol ${solTimeLabel === "nå" ? "nå" : `kl. ${solTimeLabel}`}` : ""}
            {addPinMode
